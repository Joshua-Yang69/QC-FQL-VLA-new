...
def get_default_mode_and_env(train_folder, dataset_path, checkpoint, env=None, lang_embeddings=None, prep_dm_and_deps=True, device_id=0, eval_cfg_overwrite={}):
    ...
    # --- PATCH: prefer HF checkpoint dir if provided ---
    from pathlib import Path as _PatchPath
    ckpt_obj = _PatchPath(checkpoint).expanduser()
    if ckpt_obj.is_dir() and (ckpt_obj / "config.yaml").exists():
        train_folder_path = ckpt_obj
        print(f"[patch] Using HF checkpoint dir as train_folder: {train_folder_path}")
    elif ckpt_obj.is_file() and ckpt_obj.name == "model.safetensors" and (ckpt_obj.parent / "config.yaml").exists():
        train_folder_path = ckpt_obj.parent
        print(f"[patch] Using HF parent dir as train_folder: {train_folder_path}")
    # --- END PATCH ---
    ...
    if 'rollout_lh_skip_epochs' not in def_cfg:
        def_cfg.rollout_lh_skip_epochs = 5
...
def load_mode_from_safetensor(
    ...
    # Prefer the 'model' subtree if present (OmegaConf: use `'model' in config` not hasattr)
        if isinstance(config, DictConfig) and 'model' in config:
            model_conf = OmegaConf.to_object(config.model)
        else:
            model_conf = OmegaConf.to_object(config)
        load_cfg = OmegaConf.create({**model_conf, **{"optimizer": None}, **overwrite_cfg})
    else:
